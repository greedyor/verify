package main

import (
	"errors"
	"net/http"
	"strconv"

	"github.com/gin-gonic/gin"
	"github.com/gin-gonic/gin/binding"
)

var Result = gin.H{
	"code": 1,
	"msg":  "",
	"data": nil,
}

func main() {

	r := gin.Default()

	// 获取验证码信息
	r.GET("/getVerifyData", func(c *gin.Context) {
		var result = Result
		data, err := GetVerifyData(c)
		if err != nil {
			result["msg"] = err.Error()
			result["code"] = 0
		} else {
			result["data"] = data
		}
		c.JSON(http.StatusOK, result)
	})

	// 获取验证码信息
	r.GET("/chkVerifyData", func(c *gin.Context) {
		var result = Result
		data, err := ChkVerifyData(c)
		if err != nil {
			result["msg"] = err.Error()
			result["code"] = 0
		} else {
			result["data"] = data
		}
		c.JSON(http.StatusOK, result)
	})

	r.Run(":8080")
}

// 滑块验证数据，数值统一单位:xp
type SlideVerifyData struct {
	VerifyOffset     float32 `json:"verif_offset"`     // 验证偏移量
	PositionY        float32 `json:"position_y"`       // 图形滑块Y轴位置
	PositionX        float32 `json:"position_x"`       // 图形滑块X轴位置
	SlideImage       string  `json:"slide_img"`        // 滑块图片 base64
	MainImage        string  `json:"base_img"`         // 基底图片 base64
	SlideImageWidth  int     `json:"slide_img_width"`  // 滑块图片宽度
	SlideImageHeight int     `json:"slide_img_height"` // 滑块图片高度
	BaseImageWidth   int     `json:"base_img_width"`   // 基底图片宽度
	BaseImageHeight  int     `json:"base_img_height"`  // 基底图片高度
	Token            string  `json:"token"`            // 验证 token
}

// 定义一个测试的缓存数据，正式使用需要补充逻辑
var testCacheData = SlideVerifyData{
	VerifyOffset:     1,
	PositionY:        115,
	PositionX:        158,
	SlideImage:       "data:image/jpeg;base64,",
	MainImage:        "data:image/jpeg;base64,",
	SlideImageWidth:  50,
	SlideImageHeight: 50,
	BaseImageWidth:   250,
	BaseImageHeight:  180,
	Token:            "abcdefff111222333",
}

type GetVerifyDataResponse struct {
	PositionY  float32 `json:"position_y"`
	SlideImage string  `json:"slide_img"`
	BaseImage  string  `json:"base_img"`
	Token      string  `json:"token"`
}

// 获取验证数据信息
func GetVerifyData(c *gin.Context) (res GetVerifyDataResponse, err error) {
	res.PositionY = testCacheData.PositionY
	res.SlideImage = testCacheData.SlideImage
	res.BaseImage = testCacheData.MainImage
	res.Token = testCacheData.Token
	return
}

type ChkVerifyDataRequest struct {
	PositionX string `json:"position_x"`
	Token     string `json:"token"`
}

type ChkVerifyDataResponse struct {
	IsSuccess  bool   `json:"is_success"`
	LoginToken string `json:"login_token"`
}

// 检查验证
func ChkVerifyData(c *gin.Context) (res ChkVerifyDataResponse, err error) {
	var request ChkVerifyDataRequest
	if err = c.ShouldBindWith(&request, binding.JSON); err != nil {
		return
	}

	if request.Token != testCacheData.Token {
		err = errors.New("验证失败，请重新获取验证信息")
		return
	}

	PositionX, err := strconv.ParseFloat(request.PositionX, 32)
	if testCacheData.PositionX+testCacheData.VerifyOffset >= float32(PositionX) &&
		testCacheData.PositionX-testCacheData.VerifyOffset <= float32(PositionX) {
		res.IsSuccess = true
		res.LoginToken = "test111222333"
	} else {
		err = errors.New("验证失败，请重新获取验证信息")
		return
	}

	return
}
